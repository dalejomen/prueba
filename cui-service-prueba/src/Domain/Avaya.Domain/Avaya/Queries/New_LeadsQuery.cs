...
namespace Ibero.Services.Avaya.Domain.Avaya.Queries
{
    public class New_LeadsQuery : IRequest<object>
    {
        ...
        public class Handler : IRequestHandler<New_LeadsQuery, object>
        {
            ...
            public async Task<object> Handle(New_LeadsQuery request, CancellationToken cancellationToken)
            {
                var response = new object();
                var infoDB = "";
                int valueMarking = request.Marking ? 1 : 0;
                try
                {
                    using (SqlConnection sql = new SqlConnection(_connection))
                    {
                        using (SqlCommand cmd = new SqlCommand("IBEP_SP_New_Leads", sql))
                        {
                            ...
                            cmd.Parameters.Add("@ext", SqlDbType.VarChar).Value = request.ext.ToString();
                            cmd.Parameters.Add("@telefono", SqlDbType.VarChar).Value = request.telefono.ToString();
                            if (request.descripcion is null)
                            {
                            ...
                            cmd.Parameters.Add("@estado", SqlDbType.VarChar).Value = request.Estado.ToString();
                            ...
                            cmd.Parameters.Add("@marking", SqlDbType.Int).Value = valueMarking;
                ...
                {
                    throw new DeleteFailureException(nameof(New_LeadsQuery), ex.Message, ex.Message);
                }
                return response;
            }
        }
