...
{
    public class GetConvalidacionesQuery : IRequest<object>
    {
        public string Nombre { get; set; }
        public class Handler : IRequestHandler<GetConvalidacionesQuery, object>
        {
            ...
            public async Task<object> Handle(GetConvalidacionesQuery request, CancellationToken cancellationToken)
            {
                var response = new List<ConvalidacionesModel>();
                var JsonRequest = JsonConvert.SerializeObject(request);
                try
                {
                    using (SqlConnection sql = new SqlConnection(_connection))
                    {
                        using (SqlCommand cmd = new SqlCommand("SP_CONVALIDACIONES", sql))
                        {
                            ...
                            using (var sqlReader = await cmd.ExecuteReaderAsync())
                            {
                                ...
                                {
                                    ConvalidacionesModel model = new ConvalidacionesModel();
                                    ...
                                    model.id_curso_convalidado = sqlReader.GetString(5);
                ...
                {
                    throw new DeleteFailureException(nameof(GetConvalidacionesQuery), ex.Message, ex.Message);
                }
                return response;
            }
        }
